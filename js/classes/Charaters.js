class Character {
  ...
  loseHealth(ability) {
    ...
    if (this.health >= damage) {
      ...
      Swal.fire({
        text: `${ability.owner.name} kill ${this.name}`,
  ...
  showState() {
    ...
    let info = "";
    if (this.effects.dragged) info = "Dragged";
    if (this.effects.silenced) info += (info === "" ? "" : " - ") + "Silenced";
    if (this.effects.knockedUp) info += (info === "" ? "" : " - ") + "Knocked Up";
    if (this.effects.rooted) info += (info === "" ? "" : " - ") + "Rooted";
    if (this.effects.slowed) info += (info === "" ? "" : " - ") + "Slowed";
...
class CatTON extends Character {
...
class CatAeon extends Character {
  constructor(_name, _x, _y, _isEnemy) {
...
class DogeAeon extends Character {
  constructor(_name, _x, _y, _isEnemy) {
...
class UniverseAeonCat extends Character {
  constructor(_name, _x, _y, _isEnemy) {
...
class CatTONSuperMax extends Character {
  constructor(_name, _x, _y, _isEnemy) {
    let image = images.yasuo;
    let radius = 30;
    let speed = 4.2;
...
// =============== AI Character ==================
class AutoCatTON extends CatTON {
  constructor(_name, _x, _y) {
    super(_name || "#12512", _x, _y, true);
    this.autoMove = true;
  }
}

class AutoCatAeon extends CatAeon {
  constructor(_name, _x, _y) {
    super(_name || "#12612", _x, _y, true);
    this.autoMove = true;
  }
}

class AutoDogeAeon extends DogeAeon {
  constructor(_name, _x, _y) {
    super(_name || "#1671378", _x, _y, true);
    this.autoMove = true;
  }
}

class AutoUniverseAeonCat extends UniverseAeonCat {
  constructor(_name, _x, _y) {
    super(_name || "#161237", _x, _y, true);
    this.autoMove = true;
  }
}
